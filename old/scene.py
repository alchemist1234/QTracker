...
class VideoScene(QGraphicsScene):
    ...
    def __init__(self, parent: QGraphicsView, frame_index: int, frame_data: FrameData):
        ...
        parent.sig_update_scene.connect(self.update_all)
    ...
    def update_all(self, config: SceneConfig):
        ...
        if self.trajectory_style_version != config.trajectory_style_version:
            ...
            trajectory_generator = TrajectoryGenerator(self.frame_index, config)
            trajectory_generator.sig_group_generated.connect(self.update_trajectories)
    ...
    def __update_trajectories(self, config: SceneConfig):
        ...
        self.parent.sig_scene_updated.emit(self.frame_index)

    def update_trajectories(self, frame_index:int, trajectories: dict):
        if frame_index != self.frame_index:
            print('frame index not equal')
        self.trajectory_items = trajectories
        for _, trajectory_item in trajectories.items():
            self.addItem(trajectory_item)
